...
test("service worker upgrade leaves unrelated same-origin caches untouched", async () => {
  ...
  const context = vm.createContext({
    ...
    caches: { keys: async () => ["notes-shell-v1", "dsh-remote-shell-v9", "dsh-remote-shell-v10", "dsh-remote-shell-v11"], delete: async key => deleted.push(key) },
  });
  ...
  assert.deepEqual(deleted, ["dsh-remote-shell-v9", "dsh-remote-shell-v10", "dsh-remote-shell-v11"]);
});
